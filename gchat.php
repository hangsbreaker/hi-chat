...
<div class="module">
	<ol class="discussion" id="discussion">
  	<?php
	co_open();
  	$num = mysql_num_rows(mysql_query("select * from group_messages where togid=$togrpid"));
	$numshow = 200;
	$from = ($num-$numshow)<0?0:($num-$numshow);
  	$qry = mysql_query("select * from group_messages where togid=$togrpid order by sentdt asc limit $from, $numshow");
...
<div class="msgbox">
	<table width="100%"><tr>
		<td valign="top"><input type="hidden" id="togid" value="<?php echo $togrpid;?>"/>
		<textarea id="msg" class="msg" placeholder="Your message"></textarea></td>
		<td valign="top" width="55px"><button type="button" id="gsend" class="btn send">Send</button></td>
	</tr></table>
</div>
<script>
...
function startTime(){
	timer = setInterval(function () {
		stopTimer();
		$.post('ui.php', {getgdata:true,num:nums},function(data){
			//alert(data);
			if(data != ''){
